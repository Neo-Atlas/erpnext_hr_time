...
class TestDailyFlextimeStatus(unittest.TestCase):
    brake_time_def = BrakeTimeDefinitions()

    def setUp(self):
        self.brake_time_def = BrakeTimeDefinitions()
        self.brake_time_def.insert(BrakeTime(21600, 1800), False)
        self.brake_time_def.insert(BrakeTime(32400, 2700), False)
        self.brake_time_def.insert(BrakeTime(16200, 1800), True)
        self.brake_time_def.insert(BrakeTime(21600, 2700), True)
    ...
    def test_calculate(self):
        ...
        status.calculate(self.brake_time_def, 3600, False, 1.3)

        self.assertEqual(30600, status.total_working_hours)
        self.assertEqual(0, status.brake_time_deducted)
        self.assertEqual(0.5, status.flextime_delta)
        self.assertEqual(1.8, status.time_balance)

    def test_calculate_no_checked_brake(self):
        status = FlextimeDailyStatus("", datetime.date.today(), 28_800)
        ...
        self.assertEqual(3600, status.brake_time_deducted)
        self.assertEqual(1.0, status.flextime_delta)
        self.assertEqual(2.3, status.time_balance)

    def test_calculate_brake_time_below_minium(self):
        status = FlextimeDailyStatus("", datetime.date.today(), 28_800)
        status.insert_duration(self.get_duration([8, 0], [12, 0], DurationType.WORK))
        ...
        self.assertEqual(600, status.brake_time_deducted)
        self.assertEqual(0.5, status.flextime_delta)
        self.assertEqual(1.8, status.time_balance)

    def test_calculate_brake_time_above_minium(self):
        status = FlextimeDailyStatus("", datetime.date.today(), 28_800)
        status.insert_duration(self.get_duration([8, 0], [12, 0], DurationType.WORK))
    ...
    def test_calculate_minor_brake_times(self):
        status = FlextimeDailyStatus("", datetime.date.today(), 28_800)
        ...
        status.calculate(self.brake_time_def, 3600, True, 1.3)

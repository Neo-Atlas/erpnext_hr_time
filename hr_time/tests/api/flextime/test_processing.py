...
class FlextimeProcessingTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.flextime_definition.insert(WorkdayDefinition(
            0, 28_800, datetime.timedelta(), datetime.timedelta()))
        self.flextime_definition.insert(WorkdayDefinition(
            1, 28_800, datetime.timedelta(), datetime.timedelta()))
        self.flextime_definition.insert(WorkdayDefinition(
            2, 28_800, datetime.timedelta(), datetime.timedelta()))
        self.flextime_definition.insert(WorkdayDefinition(
            3, 28_800, datetime.timedelta(), datetime.timedelta()))
        self.flextime_definition.insert(WorkdayDefinition(
            4, 21_600, datetime.timedelta(), datetime.timedelta()))
        self.flextime_definition.insert(WorkdayDefinition(
            5, 0, datetime.timedelta(), datetime.timedelta()))
        self.flextime_definition.insert(WorkdayDefinition(
            6, 0, datetime.timedelta(), datetime.timedelta()))
    ...
    def assert_worklogs_called_once_with(self, employee: str, date: datetime.date):
        ...
        """
        self.mock_get_worklogs_of_employee_on_date.assert_called_once_with(
            employee, date)
    ...
    def assert_worklogs_called_within_dates(self, employee: str, start_date: datetime.date, end_date: datetime.date):
        ...
        while current_date < end_date:
            ...
            self.mock_get_worklogs_of_employee_on_date.assert_any_call(
    ...
    def assert_worklogs_not_called_with(self, employee, date):
        ...
        """
        self.mock_get_worklogs_of_employee_on_date.assert_not_called_with(
            employee, date)
    ...
    # Mock _process_employee method
    @patch.object(FlexTimeProcessingService, '_process_employee')
    def test_process_daily_status_no_flextime_time_model(self, mock_process_employee):
        ...
        mock_process_employee.add.assert_not_called()

    # Mock _process_employee method
    @patch.object(FlexTimeProcessingService, '_process_employee')
    def test_process_daily_status_no_flextime_def_found(self, mock_process_employee):
        self.break_times.get_definitions = MagicMock(
            return_value=BreakTimeDefinitions())
    ...
    def test_process_daily_status_already_up2date(self):
        ...
        self.daily_status.get_latest_status_date = MagicMock(
            return_value=datetime.date(2023, 10, 15))
        ...
        # self.assert_worklogs_not_called_with('001','2023-10-15')
        self.daily_status.get_latest_status_date.assert_called_once()
        self.daily_status.get_latest_status_date.assert_called_once_with(
            employee)
    ...
    def test_process_daily_status_holiday(self):
        ...
        self.holidays.is_holiday.assert_called_once_with(
            datetime.date(2023, 10, 16))
        self.attendance.create = MagicMock()

        self.daily_status.add.assert_called_once()
        self.assertEqual(
            "001", self.daily_status.add.call_args.args[0].employee_id)
        self.assertEqual(datetime.date(2023, 10, 16),
                         self.daily_status.add.call_args.args[0].date)
        self.assertEqual(
            0, self.daily_status.add.call_args.args[0].total_working_hours)
        self.assertEqual(
            1.5, self.daily_status.add.call_args.args[0].time_balance)
        self.assertEqual(
            0, self.daily_status.add.call_args.args[0].target_working_time)
    ...
    def test_process_join_date_used(self):
        ...
        self.assert_worklogs_called_within_dates(
            # from join_date to today
            employee.id, datetime.date(2023, 10, 1), datetime.date(2023, 10, 5))

        self.holidays.is_holiday.assert_called()
        self.assertEqual(datetime.date(2023, 10, 1),
                         self.holidays.is_holiday.call_args_list[0].args[0])
        self.assertEqual(datetime.date(2023, 10, 2),
                         self.holidays.is_holiday.call_args_list[1].args[0])
        self.assertEqual(datetime.date(2023, 10, 3),
                         self.holidays.is_holiday.call_args_list[2].args[0])
        self.assertEqual(datetime.date(2023, 10, 4),
                         self.holidays.is_holiday.call_args_list[3].args[0])

        self.daily_status.add.assert_called()
        self.assertEqual(datetime.date(2023, 10, 1),
                         self.daily_status.add.call_args_list[0].args[0].date)
        self.assertEqual(datetime.date(2023, 10, 2),
                         self.daily_status.add.call_args_list[1].args[0].date)
        self.assertEqual(datetime.date(2023, 10, 3),
                         self.daily_status.add.call_args_list[2].args[0].date)
        self.assertEqual(datetime.date(2023, 10, 4),
                         self.daily_status.add.call_args_list[3].args[0].date)

        self.attendance.create.assert_called()
        self.assertEqual(datetime.date(2023, 10, 2),
                         self.attendance.create.call_args_list[0].args[0].date)
        self.assertEqual(datetime.date(2023, 10, 3),
                         self.attendance.create.call_args_list[1].args[0].date)
        self.assertEqual(datetime.date(2023, 10, 4),
                         self.attendance.create.call_args_list[2].args[0].date)
    ...
    def test_process_correct_target_working_time_and_balance(self):
        ...
        self.checkin.get.side_effect = [
            ...
            CheckinList([]),
            CheckinList([
                CheckinEvent("E001", datetime.datetime(
                    2023, 10, 13, 8, 0), True, False),
                CheckinEvent("E002", datetime.datetime(
                    2023, 10, 13, 10, 0), False, False)
            ]),
            CheckinList([]),
            CheckinList([])
        ]
        ...
        self.service.process_daily_status()
        self.assert_worklogs_called_within_dates(
            employee.id, datetime.date(2023, 10, 9), datetime.date(2023, 10, 16))

        self.daily_status.add.assert_called()
        self.assertEqual(datetime.date(2023, 10, 9),
                         self.daily_status.add.call_args_list[0].args[0].date)
        self.assertEqual(
            28_800, self.daily_status.add.call_args_list[0].args[0].target_working_time)
        self.assertEqual(-5.9,
                         self.daily_status.add.call_args_list[0].args[0].time_balance)

        self.assertEqual(datetime.date(2023, 10, 10),
                         self.daily_status.add.call_args_list[1].args[0].date)
        self.assertEqual(
            28_800, self.daily_status.add.call_args_list[1].args[0].target_working_time)
        self.assertEqual(-13.9,
                         self.daily_status.add.call_args_list[1].args[0].time_balance)

        self.assertEqual(datetime.date(2023, 10, 11),
                         self.daily_status.add.call_args_list[2].args[0].date)
        self.assertEqual(
            28_800, self.daily_status.add.call_args_list[2].args[0].target_working_time)
        self.assertEqual(-21.9,
                         self.daily_status.add.call_args_list[2].args[0].time_balance)

        self.assertEqual(datetime.date(2023, 10, 12),
                         self.daily_status.add.call_args_list[3].args[0].date)
        self.assertEqual(
            28_800, self.daily_status.add.call_args_list[3].args[0].target_working_time)
        self.assertEqual(-29.9,
                         self.daily_status.add.call_args_list[3].args[0].time_balance)

        self.assertEqual(datetime.date(2023, 10, 13),
                         self.daily_status.add.call_args_list[4].args[0].date)
        self.assertEqual(
            21_600, self.daily_status.add.call_args_list[4].args[0].target_working_time)
        self.assertEqual(
            7200, self.daily_status.add.call_args_list[4].args[0].total_working_hours)
        self.assertEqual(-33.9,
                         self.daily_status.add.call_args_list[4].args[0].time_balance)

        self.assertEqual(datetime.date(2023, 10, 14),
                         self.daily_status.add.call_args_list[5].args[0].date)
        self.assertEqual(
            0, self.daily_status.add.call_args_list[5].args[0].target_working_time)
        self.assertEqual(-33.9,
                         self.daily_status.add.call_args_list[5].args[0].time_balance)

        self.assertEqual(datetime.date(2023, 10, 15),
                         self.daily_status.add.call_args_list[6].args[0].date)
        self.assertEqual(
            0, self.daily_status.add.call_args_list[6].args[0].target_working_time)
        self.assertEqual(-33.9,
                         self.daily_status.add.call_args_list[6].args[0].time_balance)
        ...
        self.assertEqual(datetime.date(2023, 10, 10),
                         self.attendance.create.call_args_list[0].args[0].date)
        self.assertEqual(
            Status.Absent, self.attendance.create.call_args_list[0].args[0].status)

        self.assertEqual(datetime.date(2023, 10, 11),
                         self.attendance.create.call_args_list[1].args[0].date)
        self.assertEqual(
            Status.Absent, self.attendance.create.call_args_list[1].args[0].status)

        self.assertEqual(datetime.date(2023, 10, 12),
                         self.attendance.create.call_args_list[2].args[0].date)
        self.assertEqual(
            Status.Absent, self.attendance.create.call_args_list[2].args[0].status)

        self.assertEqual(datetime.date(2023, 10, 13),
                         self.attendance.create.call_args_list[3].args[0].date)
        self.assertEqual(
            Status.Present, self.attendance.create.call_args_list[3].args[0].status)
    ...
    def test_process_correct_vacation_no_request(self):
        ...
        self.daily_status.get_latest_status_date = MagicMock(
            return_value=datetime.date(2023, 11, 19))
        self.daily_status.get_flextime_balance = MagicMock(return_value=2.1)
        self.daily_status.add = MagicMock()
        ...
        self.attendance.get = MagicMock(
            return_value=Attendance("001", today, Status.OnLeave, None))
        self.attendance.create = MagicMock()
        ...
        self.daily_status.add.assert_called()
        self.assertEqual(datetime.date(2023, 11, 20),
                         self.daily_status.add.call_args_list[0].args[0].date)
        self.assertEqual(
            0, self.daily_status.add.call_args_list[0].args[0].target_working_time)

        self.vacation.get_approved_request.assert_called_once()
        self.assertEqual(
            "001", self.vacation.get_approved_request.call_args_list[0].args[0])
        self.assertEqual(datetime.date(
            2023, 11, 20), self.vacation.get_approved_request.call_args_list[0].args[1])
    ...
    def test_process_correct_vacation_request_half_day(self):
        ...
        self.service.process_daily_status()
        self.assert_worklogs_called_once_with(
            '001', datetime.date(2023, 11, 20))

        self.daily_status.add.assert_called()
        self.assertEqual(datetime.date(2023, 11, 20),
                         self.daily_status.add.call_args_list[0].args[0].date)
        self.assertEqual(
            14400, self.daily_status.add.call_args_list[0].args[0].target_working_time)
    ...
    def test_process_other_leave(self):
        ...
        self.attendance.get = MagicMock(
            return_value=Attendance("001", today, Status.Other, None))
        self.attendance.create = MagicMock()
        ...
        self.assertEqual(datetime.date(2023, 11, 20),
                         self.daily_status.add.call_args_list[0].args[0].date)
        self.assertEqual(
            28800, self.daily_status.add.call_args_list[0].args[0].target_working_time)
